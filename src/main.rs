...
// 代码阅读器 - 改为窗口切片渲染，避免 ANSI 转换造成的乱码与大文件卡顿
    ...
    loop {
        terminal.draw(|f| {
            ...
            let view: Vec<Line> = lines
                ...
                .map(|(idx, line)| {
                    if show_lines {
                        Line::from(format!("{:>4} │ {}", idx + 1, line))
                    } else {
                        Line::from((*line).to_string())
                    }
                })
                .collect();
            let title = format!(" {} | n: 行号 | q: 返回 ", path.display());
            f.render_widget(
                Paragraph::new(view).block(Block::default().title(title)),
                size,
            );
        })?;
        ...
        if event::poll(Duration::from_millis(100))? {
            match event::read()? {
                Event::Key(k) if k.kind == KeyEventKind::Press => match k.code {
                    ...
                    KeyCode::End => scroll_y = lines.len().saturating_sub(1) as u16,
                    _ => {}
                },
                Event::Mouse(m) => match m.kind {
            ...
            match event::read()? {
                Event::Key(k) if k.kind == KeyEventKind::Press => {
                    if app.is_search_input_mode {
                        match k.code {
                            KeyCode::Enter => app.is_search_input_mode = false,
...
fn ui_list(f: &mut Frame, app: &mut App) {
    ...
    let search_indicator = if app.is_search_input_mode || app.is_searching {
        format!(" {}  /{}", app.current_dir.display(), app.search_input)
    } else {
    ...
    let list_title = " Files ".to_string();
    ...
    let items: Vec<ListItem> = display_items
        ...
        .map(|i| {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(i.name.clone(), Style::default().fg(color)),
                Span::raw(suffix),
            ]))
        })
        .collect();
    ...
    let search_hint = if app.is_search_input_mode {
        " / 输入中… Enter确认 Esc退出"
...
fn main() {
    ...
    match args.next().as_deref() {
        Some("-h") | Some("--help") => {
            ...
            print_row("/ (斜杠)", "底部输入搜索，边输边筛选", yellow, reset);
